...
#@mock.patch("application.flask.rest.room.room_list_use_case")
@mock.patch("application.fastapi.rest.room.room_list_use_case")
def test_get(mock_use_case, client):
    mock_use_case.return_value = ResponseSuccess(rooms)

    http_response = client.get("/rooms")
    # assert json.loads(http_response.data.decode("UTF-8")) == [room_dict]
    assert http_response.json() == [room_dict]
